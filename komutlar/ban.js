...
exports.run = (client, message, args) => {
  ...
  if(!modLog[message.guild.id]) return message.reply('Lütfen **x!mod-log-ayarla** yazarak Moderasyon Kayıtları kanalı ayarlayınız!');
  if (message.mentions.users.size < 1) return message.reply('Yasaklayacağın kişiyi etiketlemelisin! \n**Örnek:** x!ban <@kişi-etiket>');
  if (reason.length < 1) return message.reply('Yasaklama sebebini yazmadın! \n**Örnek:** x!ban <@kişi-etiket> <sebep>');
