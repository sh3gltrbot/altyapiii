...
client.on("message", message => {
    const dmchannel = client.channels.find(x => x.name === "bota-yazılanlar");// 'notechdm' yazan yeri sunucunuzda bi' kanalin ismini yazýn. bota özelden yazýlanlar oradan görülecektir.
...
.setColor("GREEN")
    .setFooter("SH3GLTR", client.user.avatarURL);
...
client.on('message', message => {
if (message.content === '<@518332733588766740>') {
  const embed = new Discord.RichEmbed()
  .setTitle('ben SH3GLTR:')
  .setDescription('Prefixim `x!`')
  .setFooter('Light')
  .setColor('RANDOM')
 message.reply(embed)
}
...
client.on("message", async message => {
    ...
    if (message.guild.id == "sunucu id") {
    if (message.content.toLowerCase() === prefix + 'js') {
      let rol = message.guild.roles.find(r => r.name === "hangi rolü vercekse o");
...
client.on("message", async message => {
  ...
  if (curLevel > userData.level) {
    userData.level = curLevel;
        var user = message.mentions.users.first() || message.author;
message.channel.send(`🆙 **| ${user.username} Tebrikler! Level atladın ${userData.level}**`)
    }
...
if ("message".content === "x!müzik") {
          
          const embed = new Discord.RichEmbed()
         .setAuthor(`SH3GLTR `, client.user.avatarURL)
          .addField('__**müzik Komutlar**__','Çal\n Durdur\n geç \nsra \nses \ndevam \nduraklat \nçalan ' )
            .setColor('RANDOM')
...
client.on('message', msg => {
  if (msg.content.toLowerCase() === prefix + 'sunucular') { 
      const embed = new Discord.RichEmbed()  
  .setColor("RANDOM")
  .setTitle(" Ben şu an " + client.guilds.size + " Sunucuda " + client.guilds.reduce((a, b) => a + b.memberCount, 0).toLocaleString() + " Kullanıcı İle Hizmet Veriyorum!")
...
client.on("guildCreate", async guild => {
  ...
  console.log(`${guild.name} with invite: https://discord.gg/Yr3Xw3Y`)
});

client.on("message", msg => {
	const uyarıembed = new Discord.RichEmbed()
      .setColor(0x00AE86)
      .setDescription("" + msg.author + "  Sayın Kullanıcı Sizden Ricam -----No Reklam Yes Huzur:)----- , ")
...
client.on('message', async msg => {
   ...
   .setTitle(`:loud_sound: Şuanki Ses Seviyesi: **${serverQueue.volume}**`)
    ...
    .setColor('RANDOM'));
		  return msg.channel.sendEmbed(new Discord.RichEmbed()
    .setColor('RANDOM')
     .setTitle('SH3GLTR | Şarkı Kuyruğu')
...
function play(guild, song) {
  ...
  .setTitle("**SH3GLTR | 🎙 Müzik Başladı ✔**",`https://cdn.discordapp.com/avatars/473974675194511361/6bb90de9efe9fb80081b185266bb94a6.png?size=2048`)
